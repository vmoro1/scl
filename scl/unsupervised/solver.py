...
class PrimalDualBase:
    ...
    def initialize_state_dict(self, num_dual_variables):
        ...
        self.state_dict['Relative Error'] = []
        self.state_dict['Absolute Error'] = []
        self.state_dict['linf Error'] = []
    ...
    def diagnostics(self, csl_problem, logging_dict, epoch):
        ...
        if epoch % self.eval_every == 0:
            u_pred = csl_problem.predict(self.X_test)
            error_u_relative = np.linalg.norm(self.u_exact-u_pred, 2)/np.linalg.norm(self.u_exact, 2)
            error_u_abs = np.mean(np.abs(self.u_exact - u_pred))
            error_u_linf = np.linalg.norm(self.u_exact - u_pred, np.inf)/np.linalg.norm(self.u_exact, np.inf)

            self.state_dict['Relative Error'].append(error_u_relative)
            self.state_dict['Absolute Error'].append(error_u_abs)
            self.state_dict['linf Error'].append(error_u_linf)
    ...
    def plot(self, path_save):
        ...
        plt.figure()
        plt.plot(epochs_eval, self.state_dict['Relative Error'], color='blue')
        ...
        # plot Absolute Error
        plt.figure()
        plt.plot(epochs_eval, self.state_dict['Absolute Error'], color='blue')
        ...
        # plot linf Error
        plt.figure()
        plt.plot(epochs_eval, self.state_dict['linf Error'], color='blue')
...
class SimultaneousPrimalDualParametricSolution(SimultaneousPrimalDual):
    """This is essentially the same as SimultaneousPrimalDual but it evaluates the 
    error for multiple betas (which is required for the parametric solution)"""
    ...
    def initialize_state_dict(self, num_dual_variables):
        ...
        self.state_dict['Relative Error'] = {f'Relative Error_Beta_{beta}': [] for beta in self.data_dict.keys()}
        self.state_dict['Absolute Error'] = {f'Absolute Error_Beta_{beta}': [] for beta in self.data_dict.keys()}
        self.state_dict['linf Error'] = {f'linf Error_Beta_{beta}': [] for beta in self.data_dict.keys()}
    ...
    def diagnostics(self, csl_problem, logging_dict, epoch):
        ...
        # evaluation of model
        if epoch % self.eval_every == 0:
            for beta in self.data_dict.keys():
                X_test = self.data_dict[beta]['X_test']
                u_exact = self.data_dict[beta]['u_exact']

                u_pred = csl_problem.predict(X_test, beta)
                ...
                self.state_dict['Relative Error'][f'Relative Error_Beta_{beta}'].append(error_u_relative)
                self.state_dict['Absolute Error'][f'Absolute Error_Beta_{beta}'].append(error_u_abs)
                self.state_dict['linf Error'][f'linf Error_Beta_{beta}'].append(error_u_linf)
    ...
    def plot(self, path_save):
        ...
        for beta in self.data_dict.keys():
            # plot Relative Error
            plt.figure()
            plt.plot(epochs_eval, self.state_dict['Relative Error'][f'Relative Error_Beta_{beta}'], color='blue')
            ...
            # plot Absolute Error
            plt.figure()
            plt.plot(epochs_eval, self.state_dict['Absolute Error'][f'Absolute Error_Beta_{beta}'], color='blue')
            ...
            # plot linf Error
            plt.figure()
            plt.plot(epochs_eval, self.state_dict['linf Error'][f'linf Error_Beta_{beta}'], color='blue')
        ...
        plt.figure()
        for beta in self.data_dict.keys():
            plt.plot(epochs_eval, self.state_dict['Relative Error'][f'Relative Error_Beta_{beta}'], label=f'Relative Error Beta {beta}')
        plt.title('Relative Error')
        plt.xlabel('Epochs')
        plt.ylabel('Relative Error')
        plt.legend(frameon=True, loc='best', fontsize=12)
        ...
        plt.figure()
        for beta in self.data_dict.keys():
            plt.plot(epochs_eval, self.state_dict['Absolute Error'][f'Absolute Error_Beta_{beta}'], label=f'Absolute Error Beta {beta}')
        plt.title('Absolute Error')
        plt.xlabel('Epochs')
        plt.ylabel('Absolute Error')
        plt.legend(frameon=True, loc='best', fontsize=12)
        ...
        plt.figure()
        for beta in self.data_dict.keys():
            plt.plot(epochs_eval, self.state_dict['linf Error'][f'linf Error_Beta_{beta}'], label=f'linf Error Beta {beta}')
        plt.title('linf Error')
        plt.xlabel('Epochs')
        plt.ylabel('linf Error')
        plt.legend(frameon=True, loc='best', fontsize=12)
...
class SimultaneousPrimalDualParametricSolution_General(SimultaneousPrimalDual):
    """This is essentially the same as SimultaneousPrimalDual but it evaluates the 
    error for multiple betas (which is required for the parametric solution). Moreover,
    this is specific for the reaction diffusion PDE."""
    ...
    def diagnostics(self, csl_problem, logging_dict, epoch):
        ...
        if epoch % self.eval_every == 0:
            for pde_param in self.data_dict.keys():
                ...
                # data from convection/rd is named differently than the rest
                # TODO: fix this
                try:
