# Constrained FNO for Diffusion-Sorption with constrains per sample (e.g. PDE constraint is per sample and not an average over all samples)
...
parser.add_argument('--wandb_run_name', type=str, default='constrained_fno_diffsorp_per_sample')
...
class ConstrainedFNO_DiffSorp_PerSampleConstraints(ConstrainedStatisticalLearningProblem):
    """Constrained FNO for Diffusion-Sorption. The optimization problem is 
    ...
    def __init__(self, model, n_train_samples, args):
        ...
        self.per_sample_relative_L2_loss_fn = LpLoss(size_average=False, reduction=False)  # TODO: correct, correct shape
        self.pde_loss_fn = DiffusionSorptionPDE_Loss(method=args.pde_loss_method)
        self.per_sample_pde_loss_fn = DiffusionSorptionPDE_Loss(method=args.pde_loss_method, reduce=False)
...
def main():
    ...
    constrained_fno = ConstrainedFNO_DiffSorp_PerSampleConstraints(model, args.n_train, args)
