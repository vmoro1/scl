# FNO baseline for Burgers' equation
...
parser.add_argument('--use_burger_time_dependent', action=argparse.BooleanOptionalAction, default=True, help='Whether to use Burgers equation dataset for final time or all times.')
...
parser.add_argument('--wandb_run_name', type=str, default='fno_baseline_burgers_eq')
...
def main():
    ...
    # for 1D burgers dataset, we can't compute PDE loss (so IC and BC loss also doens't make sense)
    if not args.use_burger_time_dependent:
        assert args.data_loss_weight > 0
        assert args.pde_loss_weight == args.ic_loss_weight == args.bc_loss_weight == 0
    ...
    # use_burger_time_dependent = True ==> learn solution operator from initial condition to solution at all times
    # use_burger_time_dependent = False ==> learn solution from initial condition to solution at final time

    if args.use_burger_time_dependent:
        ...
        s = 2**13 // sub    # grid size divided by the subsampling rate

        data_path = path_base + 'data/burgers_eq/burgers_data_R10.mat'
        data_reader = MatReader(data_path)
        viscosity_coeff = -1                                # Not used since we can't compute PDE loss for 1D case
        x_data = data_reader.read_field('a')[:,::sub]       # shape: (number of samples, s). Each sample is u(0, x) values for discretized grid
        y_data = data_reader.read_field('u')[:,::sub]       # shape: (number of samples, s). Each sample is u(t_final, x) values for discretized grid
        ...
        # add positional encoding (i.e. x and y coordinates) to the input data
        grid = [0, 1]
        # grid = torch.linspace(grid[0], grid[1], s).view(1, -1)
        grid = torch.linspace(grid[0], grid[1], s + 1)[:-1].view(1, -1)

        grid_train = grid.repeat(args.n_train, 1)
        grid_test = grid.repeat(args.n_test, 1)
    ...
    pde_loss_fn = BurgersPDE_Loss(visc=viscosity_coeff, method=args.pde_loss_method)
